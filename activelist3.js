...
//console.log("token is", token.token)
  ...
  //console.log(IGLTE);
  var modv6= await fetch('https://emea5.ebmpapstneo.io:443/api/tenant/devices?pageSize=10000&page=0&type=Modbus%20V6%20Fan', {
    method: "GET",
    headers: {
      'Content-Type': 'application/json',
      'X-Authorization': token.token,      
  ...
  IGAIR= await IGAIR.json()
    //console.log(IGAIR);
  var simcard= await fetch('https://emea5.ebmpapstneo.io:443/api/tenant/devices?pageSize=10000&page=0&type=SIM%20CARD', {
    method: "GET",
    headers: {
      'Content-Type': 'application/json',
      'X-Authorization': token.token,      
  ...
  simcard= await simcard.json()
    //console.log(simcard);
  var modv5= await fetch('https://emea5.ebmpapstneo.io:443/api/tenant/devices?pageSize=10000&page=0&type=Modbus%20V5.0%20Fan', {
    method: "GET",
    headers: {
      'Content-Type': 'application/json',
      'X-Authorization': token.token,      
...
// https://emea5.ebmpapstneo.io:443/api/tenant/devices?pageSize=10000&page=0&type=AirGradient%20ONE
...
//console.log(IGLTE.totalElements)
 var allProfiles= [IGLTE, IGAIR, modv6, modv6fancy, Fan_ACE, simcard, modv5, fan_v2, IGWifi, bravo, aerasgard, temco, airG]
...
var IGLTE_Active= activeList[0], IGAIR_Active=activeList[1], modv6_Active=activeList[2], modv6fancy_Active=activeList[3], Fan_ACE_Active=activeList[4], 
simcard_Active=activeList[5], modv5_Active=activeList[6], fan_v2_Active=activeList[7], IGWifi_Active= activeList[8], bravo_Active=activeList[9], aerasgard_Active=activeList[10], temco_Active=activeList[11], airG_Active= activeList[12]
...
Fan_ACE_inactive= Fan_ACE.data.length- Fan_ACE_Active, simcard_inactive= simcard.data.length- simcard_Active, modv5_inactive= modv5.data.length- modv5_Active, fan_v2_inactive= fan_v2.data.length- fan_v2_Active, 
IGWifi_inactive= IGWifi.data.length- IGWifi_Active, bravo_inctive= bravo.data.length- bravo_Active, aerasgard_inactive = aerasgard.data.length= aerasgard.data.length- aerasgard_Active,
 temco_inactive= temco.data.length- temco_Active, airG_inactive= airG.data.length- airG_Active
...
var data = {"deviceName":"activelist","deviceType":"default", "groupName": "arvindspublic", IGLTE_Active, IGAIR_Active, modv6_Active, modv6fancy_Active,  Fan_ACE_Active, simcard_Active, modv5_Active, fan_v2_Active, IGWifi_Active,
bravo_Active, aerasgard_Active, temco_Active, airG_Active,
IGLTE_inactive, IGAIR_inactive, modv6_inactive, modv6fancy_inactive, Fan_ACE_inactive, simcard_inactive, modv5_inactive, fan_v2_inactive, IGWifi_inactive, bravo_inctive, aerasgard_inactive, temco_inactive, airG_inactive,
total_Active}
//console.log(data)
